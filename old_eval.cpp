...
int eval_position(const std::vector<piece_t*>& board, bool am_white, int game_phase, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map){
  //the main evaluation function of our engine, we need the am_white parameter for the PST of the pawns, our scale will be in centipawns
  bool my_eval = true;
  int eval = 0;
  int val_b = val_of_board(board, am_white, game_phase);
  std::cout << "val of board: " << val_b << std::endl;

  eval += val_b;

  //int score_mobility = calculate_mob(board, am_white, our_map, enemy_map);
  //eval += score_mobility;

  int my_king_safety_score = king_safety(board, my_eval, am_white, our_map, enemy_map);
  std::cout << "my_king_safety_score: " << my_king_safety_score << std::endl;
  int enemy_king_safety_score = king_safety(board, !my_eval, am_white, our_map, enemy_map);
  std::cout << "enemy_king_safety_score: " << enemy_king_safety_score << std::endl;

  eval += my_king_safety_score;
  eval -= enemy_king_safety_score; //here we minus the king safety score of the opponent as the weaker the opponent king is, the stronger the position is for us

  //int center_control = eval_center_control(board, am_white, our_map, enemy_map);
  // this function is already symmetric
  //eval += center_control;
  int my_pawn_structure_score = pawn_structure(board, my_eval, am_white, our_map, enemy_map);
  std::cout << "my_pawn_structure_score: " << my_pawn_structure_score << std::endl;

  int enemy_pawn_structure_score = pawn_structure(board, !my_eval, !am_white, our_map, enemy_map);
  std::cout << "enemy_pawn_structure_score: " << enemy_pawn_structure_score << std::endl;

  eval += my_pawn_structure_score;
  eval -= enemy_pawn_structure_score; //here we minus the enemy_pawn_structure score

  int space_score = eval_space(board, am_white, our_map, enemy_map);
  std::cout << "eval_space: " << space_score << std::endl;
  eval += space_score;
...
int val_of_board(const std::vector<piece_t*>& board, bool am_white, int game_phase){
  int tot = 0;
  int param = 1;
...
int calculate_mob(const std::vector<piece_t*>& board, bool am_white, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map){
  int score = 0;
  int param = 1;
  for(piece_t* piece : board){
...
std::vector<std::string> find_king_zone(const std::vector<piece_t*>& board, bool my_king, bool am_white){
  piece_t* king = find_our_king(board, my_king);
...
int get_attack_counter(const std::vector<piece_t*>& board, bool my_eval, bool am_white, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map, const std::vector<std::string>& king_zone){
  //this functions gets the attack counter on the king zone, we have weights for each attack depending on what type of piece is perpetrating the attack
  int attack_counter=0;
  for(piece_t* piece: board){
    std::vector<std::string> possible_moves;
    if(piece->type()!="pawn" && piece->type()!="king" && piece->my_piece==!my_eval){
      if(piece->my_piece){
       possible_moves = piece->possible_moves(board, am_white, our_map, enemy_map);
      }else{
        //swapping the value of am_white, our_map and enemy_map when the piece is not our piece
        possible_moves = piece->possible_moves(board, !am_white, enemy_map, our_map);}
      
      int mobility = possible_moves.size();
      for(std::string move: possible_moves){
        std::string shortened_move = move.substr(2, 2);
        if(std::find(king_zone.begin(), king_zone.end(), shortened_move) != king_zone.end()){
          if(piece->type()=="knight" || piece->type()=="bishop"){
            //we add a weight to the king threat depending on the type of enemy piece
            attack_counter -= 20;}
          else if(piece->type()=="rook"){
            attack_counter -= 30;}
          else{
...
int calculate_king_shield(const std::vector<piece_t*>& board, bool my_eval, bool am_white) {
    piece_t* king = find_our_king(board, my_eval);
    int shield_score = 0;

    am_white = (my_eval ? am_white : !am_white);
...
int get_mob_near_king(const std::vector<piece_t*>& board, bool my_eval, bool am_white, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map, const std::vector<std::string>& king_zone){
  int mob_near_king = 0;
  for (piece_t* piece : board) {
    std::vector<std::string> possible_moves;
        if (piece->my_piece == my_eval && piece->type() != "king"){
          if(my_eval){
           possible_moves = piece->possible_moves(board, am_white, our_map, enemy_map);
          }else{
           possible_moves = piece->possible_moves(board, !am_white, enemy_map, our_map);}
...
int king_safety(const std::vector<piece_t*>& board, bool my_eval, bool am_white, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map){
  ...
  int attack_counter = get_attack_counter(board, my_eval, am_white, our_map, enemy_map, king_zone);
  int our_mob_near_king = get_mob_near_king(board, my_eval, am_white, our_map,enemy_map, king_zone);

  return score_safety = attack_counter + king_shield_score + our_mob_near_king;
}

bool is_isolated(const std::vector<piece_t*>& board, bool my_eval, const piece_t* pawn){
  for(piece_t* piece : board){
...
bool is_doubled(const std::vector<piece_t*>& board, bool my_eval, const piece_t* pawn){
...
bool is_passed(const std::vector<piece_t*>& board, bool my_eval, const piece_t* pawn, bool am_white){
  for(piece_t* piece: board){
    if(piece->type() == "pawn" && piece->my_piece != pawn->my_piece){
...
bool is_backwards(const std::vector<piece_t*>& board, bool my_eval, piece_t* pawn, bool am_white, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map){
...
int evaluate_pawn_islands(const std::vector<piece_t*>& board, bool my_eval, bool am_white){
...
int pawn_structure(const std::vector<piece_t*>& board, bool my_eval, bool am_white, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map){
...
int eval_center_control(const std::vector<piece_t*>& board, bool am_white, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map) {
...
int eval_space(const std::vector<piece_t*>& board, bool am_white, const std::map<std::string, bool>& our_map, const std::map<std::string, bool>& enemy_map) {
    int space_score = 0;
    // Important zones for space evaluation
    ...
    // create opponent's territory: if white, end of the board, if black, start of the board 
    if (am_white) {
        for (char col = 'a'; col <= 'h'; ++col) {
            for (int row = 1; row <= 8; ++row) {
              if(row >= 5){
                opponent_territory.push_back(std::string(1, col) + std::to_string(row));
              }else{
                my_territory.push_back(std::string(1, col) + std::to_string(row));
              }
            }
        }
    } else {
    ...
    // loop over each piece, check all moves, if move in certain areas, add or substract points
    for (piece_t* piece : board) {
      std::vector<std::string> controlled_squares;
        if (piece->my_piece) {
            controlled_squares = piece->possible_moves(board, am_white, our_map, enemy_map);

            for (const std::string long_square : controlled_squares) {
              std::string square = long_square.substr(2, 2);
              if (std::find(central_squares.begin(), central_squares.end(), square) != central_squares.end()) {
                  space_score += 20; // Central squares are more valuable to me
              }else if(std::find(extended_center_squares.begin(), extended_center_squares.end(), square) != extended_center_squares.end()){
                  space_score += 10;
              }else if (std::find(opponent_territory.begin(), opponent_territory.end(), square) != opponent_territory.end()) {
                  space_score += 5; // Opponent's half is valuable to me
              }else{
          ...
          //enemy piece
            controlled_squares = piece->possible_moves(board, !am_white, enemy_map, our_map);

            for (const std::string long_square : controlled_squares) {
              std::string square = long_square.substr(2, 2);
              if (std::find(central_squares.begin(), central_squares.end(), square) != central_squares.end()) {
                  space_score -= 20; // Central squares are more valuable to the opponent
              }else if(std::find(extended_center_squares.begin(), extended_center_squares.end(), square) != extended_center_squares.end()){
                  space_score -= 10; 
              }else if (std::find(my_territory.begin(), my_territory.end(), square) != my_territory.end()) {
                  space_score -= 5; // my half is more valuable to the opponent
              }else{
                std::string type = piece->type();
                if(type == "knight" || type == "bishop"){
                  space_score -= 5;}
...
std::vector<std::pair<std::string, int>> mini_eval(const std::vector<piece_t*>& board, bool am_white, int game_phase, std::map<std::string, bool>& our_map, std::map<std::string, bool>& enemy_map, bool our_turn) {
    ...
    if (our_turn) {
        possible_moves = our_final_moves_check(board, am_white);
    } else {
        possible_moves = enemy_final_moves_check(board, am_white);
    }
